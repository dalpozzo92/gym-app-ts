...
import ExerciseInfoDrawer from '@/components/workout/ExerciseInfoDrawer';
...
// ============================================
// SETS MANAGER (Gestisce UI e Autosave con Superset/Jumpset)
// ============================================
const SetsManager = ({
  exerciseId,
  initialSets,
  ...
  previousSetsData,
  onOpenHistory,
  allDayExercises // ✅ NUOVO: tutti gli esercizi del giorno per superset/jumpset
}) => {
  const { sets, updateSet } = useAutosaveSets(exerciseId, initialSets);

  // State per bottom sheet e drawer
  ...
  // ✅ Raggruppa serie per superset/jumpset
  const setGroups = useMemo(() => {
    if (!workout_day_exercise || !allDayExercises) return [];

    // Trova il group_intensity_id dal primo set (se esiste)
    const firstSet = sets[0];
    const groupIntensityId = firstSet?.group_intensity_id;

    if (!groupIntensityId) {
      // Esercizio singolo - usa solo l'esercizio corrente
      const exercises = [{ ...workout_day_exercise, workout_exercise_sets: sets }];
      const { setGroups: groups } = groupSetsForExecution(exercises);
      return groups;
    }

    // Superset/Jumpset - trova tutti gli esercizi con lo stesso group_intensity_id
    const groupedExercises = allDayExercises
      .filter(ex => {
        // Verifica se almeno un set di questo esercizio ha lo stesso group_intensity_id
        return ex.workout_exercise_sets?.some(s => s.group_intensity_id === groupIntensityId);
      })
      .map(ex => {
        // Se è l'esercizio corrente, usa i sets aggiornati, altrimenti usa quelli del DB
        if (ex.id_workout_day_exercise === exerciseId) {
          return { ...ex, workout_exercise_sets: sets };
        }
        return ex;
      });

    const { setGroups: groups } = groupSetsForExecution(groupedExercises);
    return groups;
  }, [sets, workout_day_exercise, allDayExercises, exerciseId]);

  // ✅ Ref per tracciare i valori più recenti di ogni set
  const pendingSetsRef = useRef<Record<string, { actual_load?: number; actual_reps?: number }>>({});

  // ✅ Sincronizza ref con sets
  useEffect(() => {
    sets.forEach((s) => {
  ...
  // ✅ Handler per aggiornare una serie
  const handleUpdateSet = useCallback(
    (exerciseId: number, setNumber: number, updates: any) => {
      // Trova la serie da aggiornare
      const targetSet = sets.find(s => s.set_number === setNumber);
      if (!targetSet) return;
      ...
      // ✅ Aggiorna il ref immediatamente
      if (!pendingSetsRef.current[targetSet.setId]) {
        pendingSetsRef.current[targetSet.setId] = {};
      }
      pendingSetsRef.current[targetSet.setId][field] = value;

      updateSet({ setId: targetSet.setId, field: field as any, value });
    ...
    [sets, updateSet, workout_day_exercise]
  );
  ...
  // Handler per aprire bottom sheet
  const handleOpenSetDetail = (setGroup: SetGroup) => {
    setSelectedSetGroup(setGroup);
    setIsBottomSheetOpen(true);
  };

  // Handler per chiudere bottom sheet
  const handleCloseBottomSheet = () => {
    setIsBottomSheetOpen(false);
    setTimeout(() => setSelectedSetGroup(null), 300);
  };

  // Handler per aprire info drawer
  const handleOpenExerciseInfo = (exerciseId: number) => {
    // Trova esercizio
    const exercise = workout_day_exercise;
    if (exercise) {
      setSelectedExerciseForInfo({
        id_exercise_list: exercise.id_exercise_list,
        name: exercise.exercise_name,
        description: exercise.exercise_description,
        link_video: exercise.link_video,
        muscolar_group_name: exercise.muscolar_group_name,
        notes: exercise.notes
      });
      setIsBottomSheetOpen(false); // Chiudi bottom sheet (ma non svuotare selectedSetGroup!)
      setTimeout(() => setIsInfoDrawerOpen(true), 300); // Apri drawer
      // NON svuotare selectedSetGroup qui - lo manteniamo per la riapertura
    }
  };

  // Handler per navigazione serie
  const handleNavigatePrevious = () => {
    if (!selectedSetGroup) return;
    const currentIndex = setGroups.findIndex(sg => sg.setNumber === selectedSetGroup.setNumber);
    if (currentIndex > 0) {
      setSelectedSetGroup(setGroups[currentIndex - 1]);
    }
  };
  ...
  const handleNavigateNext = () => {
    if (!selectedSetGroup) return;
    const currentIndex = setGroups.findIndex(sg => sg.setNumber === selectedSetGroup.setNumber);
    if (currentIndex < setGroups.length - 1) {
      setSelectedSetGroup(setGroups[currentIndex + 1]);
    }
  };

  const hasPrevious = selectedSetGroup ? setGroups.findIndex(sg => sg.setNumber === selectedSetGroup.setNumber) > 0 : false;
  const hasNext = selectedSetGroup ? setGroups.findIndex(sg => sg.setNumber === selectedSetGroup.setNumber) < setGroups.length - 1 : false;
  ...
  return (
    <div style={{ padding: '0 16px' }}>
      {/* Lista compatta delle serie */}
      {setGroups.map((setGroup) => {
        const isCompleted = setGroup.exercises.every(({ set }) =>
          (set.actual_load || 0) > 0 && (set.actual_reps || 0) > 0
        );

        return (
          <SetListItem
            key={`setgroup-${setGroup.setNumber}`}
            setGroup={setGroup}
            onClick={() => handleOpenSetDetail(setGroup)}
            isCompleted={isCompleted}
          />
        );
      })}

      {/* Bottom Sheet per dettagli serie */}
      <SetDetailBottomSheet
        ...
        hasNext={hasNext}
        saveStatus={setsState[0]?.status || 'Salvato'}
        previousSetsData={previousSetsData}
      />

      {/* Drawer info esercizio */}
      <ExerciseInfoDrawer
        isOpen={isInfoDrawerOpen}
        onClose={() => setIsInfoDrawerOpen(false)}
        exercise={selectedExerciseForInfo}
        onReopenBottomSheet={() => setIsBottomSheetOpen(true)}
      />
    </div>
  );
};
...
const SingleExerciseContainer = ({
  ...
  useEffect(() => {
    const fetchPreviousSetsData = async () => {
      ...
      try {
        ...
        // Estrai solo i dati della serie precedente dall'oggetto wrapper
        const extractedData = results.map(r => r.previous_workout_exercise_set);
        setPreviousSetsData(extractedData);
      } catch (error) {
  ...
  return (
    <SetsManager
      ...
      allDayExercises={allDayExercises}
    />
  );
};
        ...
        <div style={{ paddingBottom: '80px' }}>
          {currentWorkoutDayExerciseId && (
            <SingleExerciseContainer
              ...
              allDayExercises={workout_day_exercises}
            />
          )}
        </div>
      </div>
